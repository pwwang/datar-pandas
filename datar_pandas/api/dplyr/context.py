...
@n.register(DataFrame, context=Context.EVAL)
def _n(_data: DataFrame) -> int:
    _data = getattr(_data, "_datar", {}).get("summarise_source", _data)
    return _data.shape[0]


@n.register(TibbleGrouped, context=Context.EVAL)
...
@cur_data_all.register(DataFrame, context=Context.EVAL)
def _cur_data_all(_data: DataFrame) -> Series:
    _data = getattr(_data, "_datar", {}).get("summarise_source", _data)
    return Series([_data.copy()], dtype=object)


@cur_data_all.register(TibbleGrouped, context=Context.EVAL)
...
@cur_group.register(DataFrame, context=Context.EVAL)
def _cur_group(_data: DataFrame) -> Tibble:
    _data = getattr(_data, "_datar", {}).get("summarise_source", _data)
    return Tibble(index=[0])


@cur_group.register(TibbleGrouped, context=Context.EVAL)
...
@cur_group_id.register(DataFrame, context=Context.EVAL)
def _cur_group_id(_data: DataFrame) -> int:
    return 0


@cur_group_id.register(TibbleGrouped, context=Context.EVAL)
...
@cur_group_rows.register(DataFrame, context=Context.EVAL)
