...
from typing import Sequence, List, Sequence
...
@group_data.register(DataFrame, context=Context.EVAL)
def _group_data(_data: DataFrame) -> Tibble:
    return Tibble({"_rows": group_rows(_data, __ast_fallback="normal")})


@group_data.register((TibbleGrouped, GroupBy), context=Context.EVAL)
...
@group_keys.register(DataFrame, context=Context.EVAL)
def _group_keys(_data: DataFrame) -> Tibble:
    return Tibble(index=[0])


@group_keys.register(TibbleGrouped, context=Context.EVAL)
...
@group_indices.register(DataFrame, context=Context.EVAL)
def _group_indices(_data: DataFrame) -> List[int]:
    return [0] * _data.shape[0]


@group_indices.register(TibbleGrouped, context=Context.EVAL)
