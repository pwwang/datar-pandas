...
@group_data.register(DataFrame, backend="pandas")
def _group_data(_data: DataFrame) -> Tibble:
    return Tibble({"_rows": group_rows(_data, __ast_fallback="normal")})


@group_data.register((TibbleGrouped, GroupBy), backend="pandas")
...
@group_keys.register(DataFrame, backend="pandas")
def _group_keys(_data: DataFrame) -> Tibble:
    return Tibble(index=[0])


@group_keys.register(TibbleGrouped, backend="pandas")
...
@group_indices.register(DataFrame, backend="pandas")
def _group_indices(_data: DataFrame) -> List[int]:
    return [0] * _data.shape[0]


@group_indices.register(TibbleGrouped, backend="pandas")
