...
@fct_recode.register(ForcatsRegType, context=Context.EVAL, backend="pandas")
def _fct_recode(
    _f,
...
def _fct_collapse(
    ...
    **kwargs: List,
    ...
    if other_level in kwargs:
        return fct_relevel(out, other_level, after=-1, **meta_kwargs)

    return out


@fct_lump_min.register(ForcatsRegType, context=Context.EVAL, backend="pandas")
def _fct_lump_min(
    _f,
    min: int,
    w=None,
    ...
    if other_level in new_levels:
        _f = lvls_revalue(_f, new_levels, **meta_kwargs)
        return fct_relevel(_f, other_level, after=-1, **meta_kwargs)

    return _f


@fct_lump_prop.register(ForcatsRegType, context=Context.EVAL, backend="pandas")
def _fct_lump_prop(
    _f,
    prop,
    w=None,
...
@fct_lump_n.register(ForcatsRegType, context=Context.EVAL, backend="pandas")
def _fct_lump_n(
    _f,
    n: int,
    w=None,
...
@fct_lump_lowfreq.register(
    ForcatsRegType,
    context=Context.EVAL,
    backend="pandas",
)
def _fct_lump_lowfreq(_f, other_level: Any = "Other"):
...
@fct_relabel.register(ForcatsRegType, context=Context.EVAL, backend="pandas")
def _fct_relabel(
    _f,
    _fun: Callable,
    *args: Any,
    **kwargs: Any,
    ...
    return lvls_revalue(_f, new_levels, **meta_kwargs)
...
def check_calc_levels(_f, w=None):
    ...
    if w is None:
        ...
        cnt = (
            DataFrame({"w": w, "f": _f})
            .groupby("f", observed=False, sort=False, dropna=True)
            .agg("sum")
            .iloc[:, 0]
            .values
        )
