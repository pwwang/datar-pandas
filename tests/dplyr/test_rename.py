...
def test_can_select_columns():
    df = tibble(x=1, y=2)
    out = df >> rename_with(str.upper) >> colnames()
    assert_iterable_equal(out, ["X", "Y"])
    out = df >> rename_with(str.upper, 0) >> colnames()
    assert_iterable_equal(out, ["X", "y"])
    out = df >> rename_with(str.upper, f.x) >> colnames()
    assert_iterable_equal(out, ["X", "y"])


def test_passes_args_kwargs_along():
    df = tibble(x=1, y=2)
    out = df >> rename_with(str.replace, f.x, "x", "X") >> colnames()
    assert_iterable_equal(out, ["X", "y"])
