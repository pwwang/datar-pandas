...
def test_to_functions():
    ...
    out = df >> summarise(across(everything(), mean))
    expect = tibble(x=1.0)
    assert_frame_equal(out, expect)

    out = df >> summarise(
        # across(everything(), dict(mean=mean, median=median), na_rm=True)
        across(everything(), dict(mean=mean, median=median))
    )
    expect = tibble(x_mean=1.0, x_median=NA)
    assert_frame_equal(out, expect)
...
def test_return_one_row():
    ...
    out = df >> mutate(across([], as_factor))
    assert out.equals(df)

    out = df >> mutate(y=across([], as_factor))
...
def test_if_any_all_na_handling():
    df = tibble(
        x=[True, True, True, False, False, False, NA, NA, NA],
        y=[True, False, NA, True, False, NA, True, False, NA],
    )
