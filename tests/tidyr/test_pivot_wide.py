...
def test_error_when_overwriting_existing_column():
    df = tibble(a=[1, 1], key=["a", "b"], val=[1, 2])
...
def test_nested_df_pivot_correctly():
    df = tibble(i=[1, 2, 1, 2], g=list("aabb"), d=tibble(x=c[1:5], y=c[5:9]))
...
def test_duplicated_keys_aggregated_by_values_fn():
    ...
    pv = pivot_wider(
        df, names_from=f.key, values_from=f.val, values_fn=mean
    )  # mean by default
