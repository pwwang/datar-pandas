...
from datar import f
from datar.base import NA, c, NULL
from datar.dplyr import group_by, group_vars
from datar.tibble import tibble
from datar.tidyr import drop_na
...
def test_empty_call_drops_every_row2():
    df = tibble(x=c(1, 2, NA), y=c("a", NA, "b"))
    out = drop_na(df)
    assert_frame_equal(out, tibble(x=1.0, y="a"))
...
def test_works_with_list_cols():
    ...
    assert_frame_equal(rs, tibble(x=[[1]], y=1.0))
...
def test_preserves_attributes():
    ...
    df.attrs["a"] = 10
    out = drop_na(df)
    assert_frame_equal(out, tibble(x=1.0))
