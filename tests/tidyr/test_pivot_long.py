...
from datar import f
from datar.base import c, rep, factor, rnorm, NA, is_factor
from datar.dplyr import group_by, group_vars, everything, rename
...
def test_can_pivot_all_cols_to_long():
    ...
    pv2 = pivot_longer(df, c[f.x :], names_transform=str.upper)
    assert_iterable_equal(pv2.name, ["X", "X", "Y", "Y"])
    pv3 = pivot_longer(df, c[f.x :], names_transform={"name": str.upper})
    assert_iterable_equal(pv3.name, ["X", "X", "Y", "Y"])


def test_values_interleaved_correctly():
    df = tibble(x=[1, 2], y=[10, 20], z=[100, 200])
...
def test_can_drop_missing_values():
    ...
    pv = pivot_longer(df, c[f.x :], values_drop_na=True)
...
def test_mixed_columns_are_automatically_coerced():
    ...
    pv = pivot_longer(df, c[f.x :])
    assert_(is_factor(pv.value))
    assert_iterable_equal(pv.value, ["a", "b"])


def test_can_override_default_output_column_type():
    df = tibble(x="x", y=1)
    pv = pivot_longer(df, c[f.x :], values_transform={"value": lambda x: [x]})
    assert pv.value.tolist() == [["x"], [1]]
    pv2 = pivot_longer(df, c[f.x :], values_transform=lambda x: [x])
...
def test_dot_value_can_be_any_position_in_names_to():
    ...
    value_second = pivot_longer(
        samp2, ~f.i, names_to=c("time", "_value"), names_sep="_"
    )
...
def test_errors_names_sep_names_pattern():
    ...
    with pytest.raises(ValueError):
        pivot_longer(df, ~f.id, names_to=c(".value", "n"))
...
def test_names_prefix():
    ...
    pv = pivot_longer(
        df, ~f.id, names_to=c(".value", "n"), names_sep="_", names_prefix="x_"
    )
