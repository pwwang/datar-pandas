...
from datar import f
from datar.base import c, factor, letters, nrow, NULL, NA, mean, seq, rep, dim
...
def test_empty_factor_levels_dont_affect_nest():
    df = tibble(x=factor(c("z", "a"), levels=letters), y=c[1:3])
    out = nest(df, data=f.y)
...
def test_can_unnest_mixture_of_named_and_unnamed():
    df = tibble(x="a", y=[tibble(y=c[1:3])], z=[[1, 2]])
