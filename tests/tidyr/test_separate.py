...
from datar import f
from datar.base import NA, c, FALSE, colnames
...
def test_convert_keeps_characters_as_character():
    df = tibble(x="X-1")
    out = separate(df, f.x, c("x", "y"), "-", convert={"x": str, "y": int})
...
#   suppressWarnings(
#     expect_warning(separate(df, x, c("x", "y"), extra = "error"),
#                    "deprecated")
#   )
...
def test_drops_NA_columns():
    ...
    assert_iterable_equal(colnames(out), ["y"])
    assert_iterable_equal(out.y, c(NA, "b", "d"))
